...
def GetText(queue,lat,long,startTime,endTime):
    #南京经纬度(32.05，118.77)
    startTime = getTimeStamp(startTime)
    endTime = getTimeStamp(endTime)
    #获取到的微博数量 total_number
    try:
        print client.place.nearby_timeline.get(lat=lat, long=long, count=50,range=11132,
    ...
    while(True):
        try:
            data=client.place.nearby_timeline.get(page=pagenum,lat=lat, long=long, count=50,range=11132,
            ...
            try:
                ...
                print "data2 timeout error!"
        except:
            print "data1 timeout error!"
            continue
        if data:
            lastData = data
            pagenum+=1
        ...
        try:
            ...
            if record and ('geo' in record) and record['geo'] and ('coordinates' in record['geo']):
                ...
                try:
                    ...
                    file_out.write(record['created_at'].encode('utf-8')+","+record['text'].encode('utf-8')+"\n")
                except Exception, e:
                    pass
            else:
                time.sleep(3)
...
#    long=118.77
    ...
    for point in points:
        try:
            thread.start_new_thread(GetText, (queue, point['lat'], point['longt'], starttime, endtime))
        except Exception, e:
